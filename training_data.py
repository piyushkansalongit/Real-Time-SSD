...
class TrainingData:
    ...
    def __batch_generator(self, sample_list_, transforms):
        ...
        def batch_producer(sample_queue, batch_queue):
            while True:
                ...
                if images.shape[0] < batch_queue.img_shape[0]:
                    images_norm = np.zeros(batch_queue.img_shape, dtype=np.float32)
                    labels_norm = np.zeros(batch_queue.label_shape, dtype=np.float32)
        ...
        def gen_batch(batch_size, num_workers=0):
            ...
            if num_workers > 0:
                # Set up the queues
                img_template = np.zeros((batch_size, self.preset.image_size.h, self.preset.image_size.w, 3), dtype=np.float32)
                label_template = np.zeros((batch_size, self.preset.num_anchors, self.num_classes+5), dtype=np.float32)
