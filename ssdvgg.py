...
def array2tensor(x, name):
    ...
    tensor = tf.get_variable(name=name, initializer=init,
                             shape=x.shape, use_resource=False)
    return tensor
...
class SSDVGG:
    ...
    def build_from_vgg(self, vgg_dir, num_classes, a_trous=True, progress_hook='tqdm'):
        """
    ...
    def build_optimizer_from_metagraph(self):
        ...
        self.localization_loss = sess.graph.get_tensor_by_name(
            'localization_loss/localization_loss:0')
        self.confidence_loss = sess.graph.get_tensor_by_name(
            'confidence_loss/confidence_loss:0')
        self.l2_loss = sess.graph.get_tensor_by_name('total_loss/l2_loss:0')
        self.optimizer = sess.graph.get_operation_by_name(
            'optimizer/optimizer')
    ...
    def __load_vgg(self, vgg_dir):
        ...
        for layer in layers:
            self.l2_loss += sess.graph.get_tensor_by_name(layer+'/L2Loss:0')

    def __build_vgg_mods(self):
        self.mod_pool5 = tf.nn.max_pool(
            self.vgg_conv5_3,
            ksize=[1, 3, 3, 1],
            strides=[1, 1, 1, 1],
            padding='SAME',
    ...
    def __build_vgg_mods_a_trous(self):
        ...
        with tf.variable_scope('mod_conv7'):
            ...
            w = array2tensor(mod_w, 'filter')
            b = array2tensor(mod_b, 'biases')
            x = tf.nn.conv2d(self.mod_conv6, w, strides=[
    ...
    def __build_ssd_layers(self):
        ...
        x, l2 = conv_map(self.ssd_conv10_1, 256, 3,
    ...
    def __build_classifiers(self):
        ...
        with tf.variable_scope('result'):
            ...
            self.result = tf.concat(
                [self.classifier, self.locator], axis=-1, name='result')

    def build_optimizer(self, learning_rate=0.001, weight_decay=0.0005, momentum=0.9, global_step=None):

        self.labels = tf.placeholder(tf.float32, name='labels', shape=[None, None, self.num_vars])
        ...
        with tf.variable_scope('match_counters'):
            ...
            # Shape: (batch_size)
            positives_num_safe = tf.where(tf.equal(positives_num, 0), tf.ones(
                [batch_size])*10e-15, tf.to_float(positives_num))
        ...
        with tf.variable_scope('localization_loss'):
            ...
            # per sample
            # Shape: (batch_size)
            localization_loss = tf.where(tf.equal(positives_num, 0), tf.zeros([batch_size]), tf.div(localization_loss, positives_num_safe))
    ...
    def __build_names(self):
        ...
        for i in range(len(self.preset.maps)):
            for j in range(2+len(self.preset.maps[i].aspect_ratios)):
                self.new_scopes.append(
                    'classifiers/classifier{}_{}'.format(i, j))
