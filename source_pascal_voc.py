...
class PascalVOCSource:
    ...
    def load_trainval_data(self, data_dir, valid_fraction):
        ...
        all_annot = list(set(glob(root + '/Annotations/*.xml')))
        all_annot = [path.replace("\\", "/")
                     for path in glob(root + '/Annotations/*.xml')]
        train_annot = list(set(train_annot))

        valid_annot = [
            annot for annot in all_annot if annot not in train_annot]
        valid_samples = self.__build_sample_list(
            root, valid_annot, 'valid_VOC2012')
        ...
        if valid_fraction > 0:
            if len(self.valid_samples) == 0:
                raise RuntimeError(
                    'No validation samples found in ' + data_dir)
