...
from utils import initialize_uninitialized_variables, str2bool
from utils import ImageSummary, PrecisionSummary, LossSummary
...
def main():
    ...
    parser.add_argument('--save-dir', default='pascal-voc/checkpoints',
                        help='checkpoints save location')
    ...
    parser.add_argument('--batch-size', type=int, default=8,
                        help='batch size')
    parser.add_argument('--tensorboard-dir', default="pascal-voc/tensorboard",
    ...
    parser.add_argument('--continue-training', type=str2bool, default='True',
    ...
    print('[i] Save directory:           ', args.save_dir)
    ...
    if args.continue_training:
        state = tf.train.get_checkpoint_state(args.save_dir)
        if state is None:
            print('[!] No network state found in ' + args.save_dir)
            return 1
    ...
    # Create a project directory
        ...
        try:
            print('[i] Creating directory {}...'.format(args.save_dir))
    ...
    with tf.Session() as sess:
        ...
        if start_epoch != 0:
            ...
            net.build_optimizer(learning_rate=learning_rate, global_step=global_step, weight_decay=args.weight_decay, momentum=args.momentum)

        initialize_uninitialized_variables(sess)

        # Create various helpers
        summary_writer = tf.summary.FileWriter(logdir=args.tensorboard_dir, graph=None)
        ...
        training_loss = LossSummary(sess, summary_writer, 'training', td.num_train, restore)
        validation_loss = LossSummary(sess, summary_writer, 'validation', td.num_valid, restore)
        ...
        for e in range(start_epoch, args.epochs):
            ...
            for x, y, gt_boxes in tqdm(generator, total=n_train_batches, desc=description, unit='batches'):

                if len(training_imgs_samples) < 3:
                    saved_images = np.copy(x[:3])

                feed = {net.image_input: x, net.labels: y}
                result, loss_batch, _ = sess.run([net.result, net.losses, net.optimizer], feed_dict=feed)
            ...
            for x, y, gt_boxes in tqdm(generator, total=n_valid_batches, desc=description, unit='batches'):

                feed = {net.image_input: x, net.labels: y}
                result, loss_batch = sess.run([net.result, net.losses], feed_dict=feed)
            ...
            if (e+1) % args.checkpoint_interval == 0:
                checkpoint = '{}/e{}.ckpt'.format(args.save_dir, e+1)
                saver.save(sess, checkpoint)
                print('[i] Checkpoint saved:', checkpoint)

        checkpoint = '{}/final.ckpt'.format(args.save_dir)
        saver.save(sess, checkpoint)
        print('[i] Checkpoint saved:', checkpoint)
